...
class WanisEngine:
    ...
    def _build_dynamic_tracks(self):
        # [فيتشر 3] التصنيف الديناميكي لمواد جامعة المنصورة
    ...
    def get_recommendation(self, student_dict):
        ...
        # استخدام الـ GPA في الحساب
        gpa = student_dict.get("GPA", 0.0)
        trend = 0.15 if gpa >= 3.5 else 0.10
        
        final_scores = (w1 * content_sims) + (w2 * collab_sims) + (w3 * trend)
        recs = sorted([{"course": self.course_names[i].replace("_", " "), "score": round(float(final_scores[i]), 4)} for i in range(len(self.course_names))], key=lambda x: x["score"], reverse=True)[:3]
        
        return {"dominant_track": dominant_track, "recommendations": recs}

    def retrain_model(self, url):
        # [فيتشر 4]
        return "Model Re-synced with Analytics Database"
